...
# =========================================================
# ?숋툘 Scientific Retraining Config (ESM2-15B)
# =========================================================
...
OUTPUT_DIR = './results/scientific/esm2_15b'
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Hyperparams (ESM2-15B is 5120dim)
INPUT_DIM = 5120
BATCH_SIZE = 128
LR = 1e-3
EPOCHS = 10
...
print(f"?뵦 Device: {device} | Scientific Training: ESM2-15B")
...
# =========================================================
# ?쭬 Model (Scaled for 5120 dim)
# =========================================================
class SimpleMLP(nn.Module):
    def __init__(self, input_dim, num_classes):
        super().__init__()
        # Deeper/Wider for ESM? Keep structure similar for fair comparison
        self.layers = nn.Sequential(
            nn.Linear(input_dim, 2048), # 5120 -> 2048
...
def main():
    ...
    if train_emb.shape[1] != INPUT_DIM:
         print(f"?좑툘 Warning: Dimension mismatch! Expected {INPUT_DIM}, got {train_emb.shape[1]}")
         # Adjust global INPUT_DIM for dataset?
         # Requires object change or restart. But let's assume correct.
    ...
    val_ds = AggregatedDataset(train_emb, train_id_map, val_ids, num_classes=0)
    ...
    model.eval()
    val_out_file = os.path.join(OUTPUT_DIR, "val_pred_esm.tsv")
